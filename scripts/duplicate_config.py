...
if len(sys.argv) == 2:
    ...
    mem_file = "mem_alloc.txt"
    path = ""

mem_each_tile = 16384
mem_alloc = []
alloc_file = open(mem_file, "r")
alloc_file.readline()
...
def add_config(configs, x_shift_value, y_shift_value, allo_shift_value, file_name):
    ...
    for line in config_lines:
        ...
        if "Y=" in line:
            ...
            newline = (
                "Y="
                + str(int(x) + x_shift_value)
                + " X="
                + str(int(y) + y_shift_value)
                + ","
            )
            ...
            if bits[1] == "1" and (
                (bits[29:34] == "00001" and const in mem_alloc)
                or const == 16383
                or const == 16382
            ):
                # configuration valid
...
configs = []
left_config_file = open(f"./{path}/left.bin", "r")
...
configs = add_config(configs, 0, 4, 2, f"./{path}/right.bin")
configs = add_config(configs, 4, 0, 1, f"./{path}/left.bin")
configs = add_config(configs, 4, 4, 3, f"./{path}/right.bin")
